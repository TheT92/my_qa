...
public interface PlanDao extends JpaRepository<StudyPlan, Integer> {
    ...
    List<StudyPlan> getPlansPage(String query, Integer id, Integer pageSize, Integer offset);

    @Query(value = "SELECT count(*) FROM study_plan q where q.title like %?1% and q.user_id = ?2 and q.del_flag != 1", nativeQuery = true)
    Integer getPlansPageNum(String query, Integer id);

    @Query(value = "SELECT * FROM study_plan q where q.id = :id and q.del_flag != 1", nativeQuery = true)
    StudyPlan getPlanDetail(int id);
}
